...
def login():
    ...
    try:
        ...
        # Keep response stable for the UI when DB connection/config is invalid.
        return jsonify({"message": "Server database connection failed. Please contact administrator."}), 500
    finally:
        if cur:
            cur.close()
